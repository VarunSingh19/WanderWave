...
export async function POST(req: NextRequest) {
  try {
    ...
    const userId = session.user.id;
    const { recipientIdentifier, recipientId } = await req.json();

    // Check if we have either recipientId or recipientIdentifier
    if (!recipientIdentifier && !recipientId) {
      return NextResponse.json(
        { error: "Username, email, or user ID is required" },
        { status: 400 }
      );
    }
    ...
    // Find recipient by ID if provided, otherwise by email or username
    if (recipientId) {
